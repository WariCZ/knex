...
class Runner {
  ...
  async ensureConnection(cb, cbParams) {
    ...
    try {
      ...
      // Call before trigger
      let beforeData;
      if (this.client.triggers && this.client.triggers.before) {
        beforeData = await this.client.triggers.before(this);
      }
      if (beforeData === false) {
        console.warn('Before data from trigger is false skip query');
        // return [];

        throw 'Before data from trigger is false skip query';
        // return [];
      }
      const ret = await cb(this, cbParams);

      // Call after trigger
      if (this.client.triggers && this.client.triggers.after) {
        await this.client.triggers.after(this, ret, beforeData);
      }
